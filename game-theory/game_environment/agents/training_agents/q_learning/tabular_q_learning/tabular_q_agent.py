...
class TabularQAgent(QAgent):
    ...
    def __init__(self, env:IdsGameEnv, config: QAgentConfig):
        ...
        if not self.config.tab_full_state_space:
            ...
            self.Q_attacker = np.zeros((self.env.num_states_full, self.env.num_attack_actions))  # num_states_full可能是做独热映射的
            self.Q_defender = np.zeros((self.env.num_states_full, self.env.num_defense_actions))
            if self.env.num_states_full < 10000:   # 这里这个 state_to_idx 为什么要做独热编码的映射，要搞清楚吧。。
                self.state_to_idx = self.env.build_state_to_idx_map()  # 改成public方法即可访问
