...
def run_experiment(configpath: str, random_seed: int, noconfig: bool):
    ...
    logger = util.setup_logger("tabular_q_learning_vs_tabular_q_learning-version", config.output_dir + "/results/logs/" +
                               str(random_seed) + "/",
                               time_str=time_str)
    ...
    if len(train_result.avg_episode_steps) > 0 and len(eval_result.avg_episode_steps) > 0:
        ...
        plot_csv(config, eval_csv_path, train_csv_path, None,random_seed)   # 这里我做了修改，加了一个None，就可以跑出5个随机种子，
